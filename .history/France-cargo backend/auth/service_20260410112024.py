...
async def google_auth_user(
    ...
    - If the google_id already exists → return that user.
    - If the email exists (registered via email) → link Google account.
    ...
    if not user:
        # 2. Check by email (user may have signed up with email first)
        result = await db.execute(select(User).where(User.email == email))
        user = result.scalar_one_or_none()
